...
void OnMonitoringStateCB(const ROBOT_STATE eState)
{
    ...
    case STATE_SAFE_STOP:
        ...
        break;
    case STATE_SAFE_OFF:
        if (g_bHasControlAuthority) assert(Drfl.SetRobotControl(CONTROL_SERVO_ON));
        break;
...
void OnMonitroingAccessControlCB(const MONITORING_ACCESS_CONTROL eTrasnsitControl)
{
    ...
    case MONITORING_ACCESS_CONTROL_LOSS:
        ...
        if (g_TpInitailizingComplted) {
            ...
            //Drfl.ManageAccessControl(MANAGE_ACCESS_CONTROL_FORCE_REQUEEST);
        }
        break;
    default:
        break;
...
{  
    ...
    while (bLoop) {
        ...
        case '4':
            {
                cout << "movej async start" << endl;
                float point[6] = { 30, 30, 30, 30, 30, 30 };
                assert(Drfl.MoveJAsync(point, 60, 120));
                eExample = EXAMPLE_MOVEJ_ASYNC;
            }
            break;
        case '5':
            {
                ...
                Drfl.Home(TRUE);
                /*
                cout << "movel sync start" << endl;
                float point[6] = { 756.6f, 511.6f, 876.0f, 44.5f, 86.7f, 55.7f };
                float velacc[2] = {30, 30 };
                assert(Drfl.MoveL(point, velacc, velacc));
                eExample = EXAMPLE_MOVEL_SYNC;
                cout << "movel sync stop" << endl;

                cout << "task: ";
                LPROBOT_POSE lpPose = Drfl.GetCurrentPose(ROBOT_SPACE_TASK);
                for (int k = 0; k < NUM_JOINT; k++ )
                    cout << lpPose->_fPosition[k] << ",";
                cout << endl;
                */
            }
            break;
        case '6':
            {
                cout << "movej sync start" << endl;
                float point[6] = {0,0,90,0,90,0};
                assert(Drfl.MoveJ(point, 30, 30));
                eExample = EXAMPLE_MOVEJ_SYNC;
                cout << "movej sync stop" << endl;

                cout << "joint: ";
                for (int k = 0; k < NUM_JOINT; k++ )
                    cout <<  Drfl.GetCurrentPose(ROBOT_SPACE_JOINT)->_fPosition[k] << ",";
                cout << endl;
            }
            break;
        case '7':
